...
CREATE PROCEDURE [CONNECT_MS].[dataMigrationFrom2020300To2021100]
(
	@p_retentionDays    INT,
	@p_batchSize    INT
)
...
BEGIN
    ...
    SELECT DISTINCT MH.SRVC_TYPE, MH.SRVC_NAME, MH.SRVC_VER, MH.SRVC_INST, MH.SRVC_HOST , CONVERT(VARCHAR(256),HASHBYTES('SHA2_256',(MH.SRVC_TYPE + MH.SRVC_NAME + MH.SRVC_VER + MH.SRVC_INST + MH.SRVC_HOST)),2) 
	...
	BEGIN 
		...
		BEGIN
			...
			SELECT (CAST(MSG_BDY_ID AS BIGINT)), CRTD_AT , @now, MSG_FRMT, (CAST(MSG_BDY_ID AS VARCHAR)), (COMPRESS(MSG_BDY)), 1
			FROM dbo.MS_MSG_BDY
	...
	BEGIN  -- (@lv_BatchSize > 0)
		...
		BEGIN
            ...
            ON CONVERT(VARCHAR(256),HASHBYTES('SHA2_256',(MH.SRVC_TYPE + MH.SRVC_NAME + MH.SRVC_VER + MH.SRVC_INST + MH.SRVC_HOST)),2) = MS.D_DUP_KEY
