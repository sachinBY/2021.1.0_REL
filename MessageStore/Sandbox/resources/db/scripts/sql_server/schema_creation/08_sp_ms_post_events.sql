...
BEGIN
	...
	BEGIN TRANSACTION
		INSERT INTO MS_MSG_EVNT(CRTD_AT, MSG_HDR_ID, STATUS, ERR_CAT, ERR_REASON, ERR_CODE, ERR_DESC, ERR_DETAIL_DESC)
			VALUES(@now, @headerId, @status, @errorCategory, @errorReason, @errorCode, @errorDesc, @errorDetailedDesc)
				...
				IF (@@ROWCOUNT = 0)
					BEGIN
						INSERT INTO MS_RPLYBL_MSGS(CRTD_AT,LST_UPDT_AT,MSG_STORE_ID,NXT_RUN_AT)
							VALUES
							(@now,@now,@headerId,@now)						
					END
			END
		ELSE 
			BEGIN
				DELETE FROM MS_RPLYBL_MSGS WHERE MSG_STORE_ID = @headerId
