...
COMMENT ON TABLE &SCHEMANAME.MS_SRVC IS
    'Table that holds information about Services associated Messages';
	
ALTER TABLE &SCHEMANAME.MS_SRVC ADD CONSTRAINT MS_SRVC_ID_PK PRIMARY KEY (SRVC_ID);

ALTER TABLE &SCHEMANAME.MS_SRVC ADD CONSTRAINT MS_SVC_DDUP_UK UNIQUE ( D_DUP_KEY );
...
COMMENT ON TABLE &SCHEMANAME.MS_MSG_BDY IS
    'Table that saves the  body of a  Message';

ALTER TABLE &SCHEMANAME.MS_MSG_BDY ADD CONSTRAINT MSG_BDY_ID_PK PRIMARY KEY ( MSG_BDY_ID );

ALTER TABLE &SCHEMANAME.MS_MSG_BDY ADD CONSTRAINT MS_BDY_DDUP_UK UNIQUE ( D_DUP_KEY, ACTIVE );
...
COMMENT ON TABLE &SCHEMANAME.MS_MSG_EVNT IS
    'Table which contains events associated with a Message';

ALTER TABLE &SCHEMANAME.MS_MSG_EVNT ADD CONSTRAINT MSG_EVNT_ID_PK PRIMARY KEY ( MSG_EVNT_ID );

ALTER TABLE &SCHEMANAME.MS_MSG_EVNT ADD CONSTRAINT MS_EVNT_HDR_CRTD_UK UNIQUE (MSG_HDR_ID, CRTD_AT);
...
COMMENT ON TABLE &SCHEMANAME.MS_MSG_HDR IS
    'Table that saves Header level information associated to a Message';

ALTER TABLE &SCHEMANAME.MS_MSG_HDR ADD CONSTRAINT MSG_HDR_ID_PK PRIMARY KEY ( MSG_HDR_ID );

ALTER TABLE &SCHEMANAME.MS_MSG_HDR ADD CONSTRAINT MSG_HDR_DDUP_UK UNIQUE ( D_DUP_KEY );
...
COMMENT ON TABLE &SCHEMANAME.MS_BLK_EVNT IS
    'Table that holds information about  events associated with  Bulk Messages';

ALTER TABLE &SCHEMANAME.MS_BLK_EVNT ADD CONSTRAINT BLK_EVNT_ID_PK PRIMARY KEY ( BLK_EVNT_ID );
...
COMMENT ON TABLE &SCHEMANAME.MS_BLK_HDR IS
    'Table that holds Bulk level Message Header information.';

ALTER TABLE &SCHEMANAME.MS_BLK_HDR ADD CONSTRAINT BLK_HDR_ID_PK PRIMARY KEY ( BLK_HDR_ID );

ALTER TABLE &SCHEMANAME.MS_BLK_HDR ADD CONSTRAINT BLK_HDR_DTLS_UK UNIQUE ( SRVC_NAME, BLK_SRC, BLK_ID );
...
COMMENT ON TABLE &SCHEMANAME.MS_RCRD_EVNT IS
    'Table that saves information about events associated with a Record';

ALTER TABLE &SCHEMANAME.MS_RCRD_EVNT ADD CONSTRAINT BLK_RCRD_EVNT_ID_PK PRIMARY KEY ( BLK_RCRD_EVNT_ID );
...
COMMENT ON TABLE &SCHEMANAME.MS_RCRD_HDR IS
    'Table that saves  Record level information. A bulk is made of one or more records';

ALTER TABLE &SCHEMANAME.MS_RCRD_HDR ADD CONSTRAINT BLK_RCRD_HDR_ID_PK PRIMARY KEY ( BLK_RCRD_HDR_ID );

ALTER TABLE &SCHEMANAME.MS_RCRD_HDR ADD CONSTRAINT BLK_HDRRCRD_ID_UK UNIQUE ( BLK_HDR_ID, RCRD_ID );
...
COMMENT ON TABLE &SCHEMANAME.MS_VER IS
    'Table to save schema version information';

ALTER TABLE &SCHEMANAME.MS_VER ADD CONSTRAINT MSG_VER_UK UNIQUE ( VER );

ALTER TABLE &SCHEMANAME.MS_RCRD_HDR ADD CONSTRAINT BLK_RCRD_HDRID_FK FOREIGN KEY ( BLK_HDR_ID ) REFERENCES &SCHEMANAME.MS_BLK_HDR ( BLK_HDR_ID ) ON DELETE CASCADE;

ALTER TABLE &SCHEMANAME.MS_RCRD_EVNT ADD CONSTRAINT BLK_RCRDHDR_ID_FK FOREIGN KEY ( RCRD_HDR_ID ) REFERENCES &SCHEMANAME.MS_RCRD_HDR ( BLK_RCRD_HDR_ID ) ON DELETE CASCADE;

ALTER TABLE &SCHEMANAME.MS_BLK_EVNT ADD CONSTRAINT BLK_HDRID_FK FOREIGN KEY ( BLK_HDR_ID ) REFERENCES &SCHEMANAME.MS_BLK_HDR ( BLK_HDR_ID ) ON DELETE CASCADE;


/*
Add current version
*/
INSERT INTO &SCHEMANAME.MS_VER(VER, CRTD_AT, PATCH_VER) VALUES('2021.1.0', CURRENT_TIMESTAMP, 0);
COMMIT;
...
COMMENT ON TABLE &SCHEMANAME.MS_RPLYBL_MSGS IS
    'Table that saves Replay level information associated to a Message';

ALTER TABLE &SCHEMANAME.MS_RPLYBL_MSGS ADD CONSTRAINT RPLYBL_MSG_ID_PK PRIMARY KEY ( MSG_STORE_ID );
...
COMMENT ON TABLE &SCHEMANAME.MS_INGSTN_RCRD IS
...
COMMENT ON TABLE &SCHEMANAME.MS_BLK_INGSTN IS
...
ALTER TABLE &SCHEMANAME.MS_BLK_INGSTN ADD CONSTRAINT MS_BULK_INGSTN_FK FOREIGN KEY ( BLK_INGSTN_RCRD_ID ) REFERENCES &SCHEMANAME.MS_INGSTN_RCRD ( INGSTN_RCRD_ID ) ON DELETE CASCADE;
...
COMMENT ON TABLE &SCHEMANAME.MS_BLK_INGSTN_EVNT IS
    'Table that captures the ingestion events of a service';

ALTER TABLE &SCHEMANAME.MS_BLK_INGSTN_EVNT ADD CONSTRAINT MS_BULK_INGSTN_EVNT_FK FOREIGN KEY ( BLK_INGSTN_ID ) REFERENCES &SCHEMANAME.MS_BLK_INGSTN ( BLK_INGSTN_ID ) ON DELETE CASCADE;
...
ALTER TABLE &SCHEMANAME.MS_TAG ADD CONSTRAINT TAG_ID_PK PRIMARY KEY ( TAG_ID );

ALTER TABLE &SCHEMANAME.MS_TAG ADD CONSTRAINT TAG_UK UNIQUE ( TAG_NAME );

COMMENT ON TABLE &SCHEMANAME.MS_TAG IS
...
COMMENT ON TABLE &SCHEMANAME.MS_MSG_TAG IS
