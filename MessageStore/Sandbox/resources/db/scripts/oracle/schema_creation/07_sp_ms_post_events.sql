...
CREATE OR REPLACE PROCEDURE &SCHEMANAME.ms_post_events
(
p_now TIMESTAMP,
p_headerId NUMBER,
p_status VARCHAR2 ,
...
BEGIN
 SAVEPOINT evnt_store;
		INSERT INTO MS_MSG_EVNT(CRTD_AT, MSG_HDR_ID, STATUS, ERR_CAT, ERR_REASON, ERR_CODE, ERR_DESC, ERR_DETAIL_DESC)
			VALUES(p_now, p_headerId, p_status, p_errorCategory, p_errorReason, p_errorCode, p_errorDesc, p_errorDetailedDesc) returning MSG_EVNT_ID into p_eventId;
		...
		IF (p_isReplayable = 1)
			THEN
				...
				NXT_RUN_AT = p_now, 
				RPLY_COUNT = 0 
				WHERE MSG_STORE_ID = p_headerId;
				IF (SQL%ROWCOUNT = 0)
					THEN
						INSERT INTO MS_RPLYBL_MSGS(CRTD_AT,LST_UPDT_AT,MSG_STORE_ID,NXT_RUN_AT)
							VALUES
							(p_now,p_now,p_headerId,p_now);						
					END IF;

		ELSE 
				DELETE FROM MS_RPLYBL_MSGS WHERE MSG_STORE_ID = p_headerId;
