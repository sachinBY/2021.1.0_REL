...
--- when specifying the SOURCE_USER we should provide the dot(.) at the of the SOURCE_USER in POPUP.
DEFINE SOURCE_USER = '&SOURCE_USER';
DEFINE SCHEMANAME = 'CONNECT_MS.';

DROP TABLE &SCHEMANAME.messageBodiesToMigrate;
DROP TABLE &SCHEMANAME.bodyToProcess;
DROP TABLE &SCHEMANAME.messageHeadersToMigrate;
DROP TABLE &SCHEMANAME.headerToProcess;

CREATE GLOBAL TEMPORARY TABLE &SCHEMANAME.messageBodiesToMigrate (msg_bdy_id VARCHAR2(100)) ON COMMIT PRESERVE ROWS;
CREATE GLOBAL TEMPORARY TABLE &SCHEMANAME.bodyToProcess (msg_bdy_id VARCHAR2(100)) ON COMMIT PRESERVE ROWS;
CREATE GLOBAL TEMPORARY TABLE &SCHEMANAME.messageHeadersToMigrate (msg_hdr_id VARCHAR2(100)) ON COMMIT PRESERVE ROWS;
CREATE GLOBAL TEMPORARY TABLE &SCHEMANAME.headerToProcess(msg_hdr_id VARCHAR2(100)) ON COMMIT PRESERVE ROWS;

CREATE PROCEDURE &SCHEMANAME.dataMigrationFrom2020300To2021100
(
...
BEGIN
    ...
    -- execute immediate 'DROP TABLE ' || messageBodiesToMigrate ;
    
	lv_StartTime := SYSDATE;
	-- execute immediate 'DROP TABLE ' || messageBodiesToMigrate
    -- Migrate Data to Services  Table a
	INSERT INTO CONNECT_MS.MS_SRVC(SRVC_TYPE, SRVC_NAME, SRVC_VER, SRVC_INST, SRVC_HOST, D_DUP_KEY) 
    SELECT DISTINCT MH.SRVC_TYPE, MH.SRVC_NAME, MH.SRVC_VER, MH.SRVC_INST, MH.SRVC_HOST, STANDARD_HASH(MH.SRVC_TYPE || MH.SRVC_NAME || MH.SRVC_VER || MH.SRVC_INST || MH.SRVC_HOST) FROM &SOURCE_USER.MS_MSG_HDR MH
	...
	-- Migrate Tags
	-- IDENTITY_INSERT CONNECT_MS.MS_TAG ON
    INSERT INTO CONNECT_MS.MS_TAG ("TAG_ID", "TAG_NAME", "CREATED_AT")
	SELECT (CAST(TAG_ID AS SMALLINT)), TAG_NAME, CREATED_AT FROM &SOURCE_USER.MS_TAG;
	...
	--IDENTITY_INSERT CONNECT_MS.MS_TAG OFF
    
    	-- Migrate Transactional Tables 
	-- Migrate Bodies 
	
	-- Select body id's to be migrated into temp table
	INSERT INTO messageBodiesToMigrate(msg_bdy_id)
	SELECT msg_bdy_id FROM &SOURCE_USER.MS_MSG_HDR MH
	  WHERE CAST(MH.LST_UPDT_AT AS DATE) > CAST((SYSDATE - p_retentionDays) AS DATE)
    ORDER BY msg_bdy_id;
	...
	-- IDENTITY INSERT ON as we will be inserting identities from source table
	-- IDENTITY_INSERT CONNECT_MS.MS_MSG_BDY ON
    ...
    LOOP
        BEGIN
            ...
            INSERT INTO bodyToProcess(msg_bdy_id)
            SELECT msg_bdy_id
            FROM messageBodiesToMigrate
            ORDER BY messageBodiesToMigrate.msg_bdy_id
			...
			SELECT CAST(MSB.MSG_BDY_ID AS NUMBER), MSB.CRTD_AT, currentDateTime, MSB.MSG_FRMT, CAST(MSB.MSG_BDY_ID AS VARCHAR2(20)),clob_to_blob(MSG_BDY), 1  FROM &SOURCE_USER.MS_MSG_BDY MSB
            WHERE MSB.MSG_BDY_ID IN (SELECT msg_bdy_id FROM bodyToProcess);
			...
			DELETE FROM messageBodiesToMigrate WHERE msg_bdy_id IN (SELECT msg_bdy_id FROM bodyToProcess);
	...
	INSERT INTO messageHeadersToMigrate(msg_hdr_id)
	...
	BEGIN  -- (lv_BatchSize > 0)
        ...
        LOOP
            BEGIN
                ...
                -- SELECT batch size into temporary table
                INSERT INTO headerToProcess(msg_hdr_id)
                SELECT msg_hdr_id FROM messageHeadersToMigrate
                ORDER BY msg_hdr_id;
                ...
                END IF;   
                -- IDENTITY INSERT ON
                -- IDENTITY_INSERT CONNECT_MS.MS_MSG_HDR ON
                ...
                ON STANDARD_HASH(MH.SRVC_TYPE || MH.SRVC_NAME || MH.SRVC_VER || MH.SRVC_INST || MH.SRVC_HOST) = MS.D_DUP_KEY
                WHERE MH.MSG_HDR_ID IN (SELECT msg_hdr_id FROM headerToProcess);
                ...
                --IDENTITY_INSERT CONNECT_MS.MS_MSG_HDR OFF
    
                -- Migrate EVENTS
    
                --IDENTITY_INSERT CONNECT_MS.MS_MSG_EVNT ON
                ...
                WHERE ME.MSG_HDR_ID IN (SELECT msg_hdr_id FROM headerToProcess);
                ...
                --IDENTITY_INSERT CONNECT_MS.MS_MSG_EVNT OFF
                ...
                WHERE MT.MSG_HDR_ID IN (SELECT msg_hdr_id FROM headerToProcess);
                ...
                WHERE RM.MSG_STORE_ID IN (SELECT msg_hdr_id FROM headerToProcess);
                ...
                DELETE FROM messageHeadersToMigrate WHERE msg_hdr_id IN (SELECT msg_hdr_id FROM headerToProcess);
